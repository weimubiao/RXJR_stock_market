...
public class StockProTopology {
    ...
    public static void main(String[] args) throws TException {
        ...
        spoutConfig.zkServers = Arrays.asList(new String[] {"HQ01","HQ02","HQ03"});
        ...
        builder.setSpout("kafka-reader4", new KafkaSpout(spoutConfig), 2)
                .setNumTasks(2);
        // 设置数据处理节点名称，实例，并行度。
         /**
         * 存入hbase
         */
        builder.setBolt("splitter4", new StockHQSplitBolt(), 2).shuffleGrouping("kafka-reader4").setNumTasks(2);
        //builder.setBolt("processor", new ProcessingBolt(), 2).fieldsGrouping("splitter", new Fields("area")).setNumTasks(2); // 相同区域交由同一个bolt处理
        builder.setBolt("writer4",new HBaseHQBolt(),1).fieldsGrouping("splitter4",new Fields("row")).setNumTasks(1); // 相同key交由同一个bolt处理
        ...
        String name = StockProTopology.class.getSimpleName();
